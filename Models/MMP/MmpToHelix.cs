...
namespace RHToolkit.Models.MMP
{
    ...
    {
        public static IEnumerable<Element3D> CreateNodes(MmpModel model)
        {
            foreach (var obj in model.Objects)
            {
                ...
                foreach (var part in obj.Parts)
                {
                    ...
                    var positions = new Vector3Collection(part.Vertices.Select(v =>
                        new SDX.Vector3(v.Position.X, v.Position.Y, v.Position.Z)));
                    var normals = new Vector3Collection(part.Vertices.Select(v =>
                        new SDX.Vector3(v.Normal.X, v.Normal.Y, v.Normal.Z)));
                    ...
                    var texcoords = new Vector2Collection(part.Vertices.Select(v =>
                        new SDX.Vector2(v.UV0.U * x, v.UV0.V * y)));
        ...
        private static MmpShader? Shader(MmpMaterial m, string slotPrefix)
            => m.Shaders.FirstOrDefault(s => s.Slot.StartsWith(slotPrefix, System.StringComparison.OrdinalIgnoreCase));

        private static MmpTexture? Texture(MmpMaterial m, string slotExact)
            => m.Textures.FirstOrDefault(t => t.Slot.Equals(slotExact, System.StringComparison.OrdinalIgnoreCase));
