...
public class CRC32
{
    ...
    public static async Task<uint> ComputeCrc32HashAsync(string filePath, CancellationToken ct)
    {
        int num = -1;
        ...
        while ((bytesRead = await stream.ReadAsync(buffer.AsMemory(0, bufferSize), ct)) > 0)
        {
            ...
            {
                num = (int)Crc32Table[num & 0xff ^ buffer[i]] ^ (num >> 8 & 0xffffff);
            }
        }
    ...
    /// </summary>
    /// <param name="filePath"></param>
    /// <returns>CRC32 hash as a hexadecimal string.</returns>
